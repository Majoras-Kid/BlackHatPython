...
def main():
	...
	if not listen and len(target) and port > 0:
		#read in the buffer from the commandlide
		#this will block, so send CTRL-D if not sending input
		#to stdin
		print "Reading buffer from CMDLine"
		buffer = sys.stdin.read()

		#send data off
		client_sender(buffer)

	#we are going to to listen and potentially
	#upload thiings, execute commands, and drop a shell back
